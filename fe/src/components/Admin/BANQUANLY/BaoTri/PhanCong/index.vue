...
async function fetchData() {
  ...
  try {
    ...
    pendingRequests.value = requestRows.filter(r => r.trang_thai === 'da_xac_nhan');
    
    // 2. Fetch technicians for assignment
    const resUsers = await api.get('/technicians');
    if (resUsers.data) {
      technicians.value = resUsers.data.filter(u => u.vai_tro === 'nhan_vien' || u.vai_tro === 'technician');
    }
  } catch (error) {
...
async function submitAssignment() {
  ...
  try {
    ...
    await api.post('/phan-cong', {
      ...
      trang_thai: 'cho_thuc_hien' // Trạng thái mặc định của phân công
    });
    ...
    // 3. Remove item from list locally or refetch
    await fetchData();
    closeModal();
...
function getPriorityLabel(val) {
  if (val === 'gan') return 'Khẩn cấp';
  if (val === 'binh_thuong') return 'Bình thường';
  if (val === 'kho') return 'Ưu tiên thấp';
  return 'Chưa rõ';
}

function getPriorityClass(val) {
  if (val === 'gan') return 'badge-danger';
  if (val === 'binh_thuong') return 'badge-success';
  if (val === 'kho') return 'badge-warning';
