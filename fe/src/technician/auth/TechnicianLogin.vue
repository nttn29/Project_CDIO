...
async function onLogin() {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/technician/login`, {
      method: "POST",
    ...
    localStorage.setItem("tech_user", JSON.stringify(data?.data || null));
...
async function onRegister() {
  ...
  try {
    submitting.value = true;
    const res = await fetch(`${API_BASE}/api/technician/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
