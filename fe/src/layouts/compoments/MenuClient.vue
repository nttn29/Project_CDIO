            ...
            <div class="navbar-menu" :class="{ 'is-active': isMobileMenuOpen }">
                <ul class="menu-list">
                    <li v-for="(item, index) in menuItems" :key="index" @click="closeMobileMenu">
                        <a :href="item.link" class="menu-link" :class="{ 'active': activeIndex === index }"
...
export default {
    ...
    data() {
        return {
            ...
            menuItems: [
                { text: "Trang chủ", link: "/", icon: "fas fa-home" },
                { text: "Báo cáo sự cố", link: "/bao-cao", icon: "fas fa-tools" },
                { text: "Hóa đơn", link: "/hoa-don", icon: "fas fa-file-invoice", badge: "1" }, // Có 1 hóa đơn chưa đóng
                { text: "Tiện ích", link: "/tien-ich", icon: "fas fa-swimming-pool" },
                { text: "Cộng đồng", link: "/tin-tuc", icon: "fas fa-users" },
            ]
        };
    },
    mounted() {
